...
WEBHOOK_SECRET = BOT_TOKEN[:16]  # استخدام جزء من التوكن كسري
...
def debug_info():
    ...
    return json.dumps({
        "webhook_url": WEBHOOK_URL,
        "webhook_base": WEBHOOK_URL_BASE,
        "bot_username": bot.get_me().username if bot else "unknown",
        "active_bots": len(active_bots) if 'active_bots' in globals() else 0,
        "timestamp": datetime.now().isoformat()
    }), 200, {'Content-Type': 'application/json'}
...
def load_all_bots():
    ...
    logging.info(f"📊 تم تحميل {loaded_count} بوت نشط")
    return loaded_count
...
def main():
    """الدالة الرئيسية"""
    print("="*70)
    print("🤖 TELEBOT MULTI-USER SYSTEM WITH WEBHOOK SUPPORT")
    print("="*70)
    ...
    print(f"✅ تم تحميل {loaded_bots} بوت نشط")
