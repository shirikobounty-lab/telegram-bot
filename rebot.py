...
def load_tested_numbers():
    ...
    if os.path.exists(TESTED_NUMBERS_FILE):
        with open(TESTED_NUMBERS_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    return {
        "numbers": {},  # {message_id: {"user_id": "", "tested_at": "", "number_text": ""}}
        "by_user": {}   # {user_id: [message_ids]}
    }
...
def is_number_tested(source_message_id):
    """التحقق إذا كان الرقم مجرب من قبل"""
    tested_data = load_tested_numbers()
    return str(source_message_id) in tested_data["numbers"]

def mark_number_as_tested(user_id, source_message_id, number_text):
    """تسجيل رقم كمجرب"""
    tested_data = load_tested_numbers()
    
    # إضافة للقائمة
    tested_data["numbers"][str(source_message_id)] = {
        "user_id": user_id,
        "tested_at": datetime.now().isoformat(),
        "number_text": number_text[:200]
    }
    ...
    if source_message_id not in tested_data["by_user"][user_id]:
        tested_data["by_user"][user_id].append(source_message_id)
    
    save_tested_numbers(tested_data)
    
    # تحديث الإحصائيات
    update_tested_stats(user_id, {"tested_at": datetime.now().isoformat()})
...
def cleanup_old_tested_numbers(days=30):
    ...
    for msg_id, info in tested_data["numbers"].items():
        tested_time = datetime.fromisoformat(info["tested_at"])
        if (current_time - tested_time) > timedelta(days=days):
            to_delete.append(msg_id)
    
    for msg_id in to_delete:
        user_id = tested_data["numbers"][msg_id]["user_id"]
        if user_id in tested_data["by_user"] and msg_id in tested_data["by_user"][user_id]:
            tested_data["by_user"][user_id].remove(msg_id)
        del tested_data["numbers"][msg_id]
...
def extract_number_from_text(text):
    """استخراج الرقم من النص للإحصائيات"""
    # محاولة استخراج أول 100 حرف كعينة
    ...
    def handle_channel_post(self, message):
        ...
        try:
            ...
            if condition_met:
                ...
                if is_number_tested(message.message_id):
...
def handle_callback_query(call):
    ...
    if callback_data in button_states:
        ...
        if button_info["status"] == "untested":
            ...
            # تسجيل الرقم في الفلتر (الأرقام المجربة)
            source_message_id = button_info["source_message_id"]
            number_text = button_info.get("number_text", "")
            mark_number_as_tested(button_info["user_id"], source_message_id, number_text)
            ...
            try:
                ...
                bot.answer_callback_query(
                    call.id,
                    text="✅ تم تحديث الحالة إلى 'مجرب' وتم حفظه في الفلتر",
                    show_alert=False
                )
    ...
    try:
        ...
        try:
            bot.send_message(
                ...
                f"   - يتحول إلى '✅ مجرب'\n"
                f"   - يتم حفظ الرقم في الفلتر\n"
                f"   - لن يتم إرسال الرقم مرة أخرى\n\n"
...
def filter_command(message):
    ...
    if user_id in tested_data["by_user"]:
        user_tested = tested_data["by_user"][user_id]
        filter_text = f"🔬 **الأرقام المجربة (الفلتر):** {len(user_tested)}\n\n"
        
        # عرض آخر 10 أرقام مجربة
        count = 0
        for msg_id in reversed(user_tested[-10:]):
            if msg_id in tested_data["numbers"]:
                info = tested_data["numbers"][msg_id]
                filter_text += f"• 🆔 {msg_id}\n"
                filter_text += f"  ⏰ {info['tested_at'][:19]}\n"
                if info.get('number_text'):
                    filter_text += f"  📝 {info['number_text'][:50]}...\n"
                filter_text += "\n"
                count += 1
        
        filter_text += f"\n📊 إجمالي الأرقام المجربة: {len(user_tested)}"
...
def status_command(message):
    ...
    if user_id in active_bots:
        ...
        if user_config:
            ...
            # الأرقام المجربة
            tested_data = load_tested_numbers()
            user_tested_count = len(tested_data["by_user"].get(user_id, []))
...
def stats_command(message):
    ...
    stats_text += f"🔬 **مجرب (فلتر):** {user_stats.get('tested', 0)}\n\n"
    ...
    if user_stats.get('total', 0) > 0:
        ...
        tested_percent = (user_stats.get('tested', 0) / user_stats['total']) * 100
...
• يتحول الزر إلى "✅ مجرب"
• يتم حفظ الرقم في الفلتر
• إذا ظهر الرقم مرة أخرى في المصدر
• لن يتم إرساله للهدف مرة أخرى

📊 **أنواع الأرقام:**
• ✅ بدون جلسة - أرقام جديدة
• 📱 تم الوصول - أرقام مفتوحة
• 🔬 مجرب - أرقام تم اختبارها
...
• الفلتر يمنع تكرار الأرقام المجربة
...
def health_monitor():
    ...
    while True:
        try:
            ...
            if datetime.now().hour == 3:  # الساعة 3 صباحاً
                cleanup_old_tested_numbers(30)
