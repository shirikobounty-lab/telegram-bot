            ...
            try:
                ...
                if user_config and user_config.get("status") == "active":
                    ...
                    bot.send_message(
                        ...
                        f"🔬 الأرقام المجربة: {len(tested_numbers)}\n\n"
...
class UserBot:
    ...
    def __init__(self, user_id, source_channel, target_channel):
        ...
        if user_id not in config["active_users"]:
            ...
            save_config(config)
            self.logger.info(f"📝 تم تسجيل المستخدم {user_id} في config.json")
...
def load_all_bots():
    ...
    logging.info(f"📋 المستخدمين في config: {config['active_users']}")
    
    # أولاً: تحميل المستخدمين من config.json
    for user_id in config["active_users"]:
        ...
        if user_config and user_config.get("status") == "active":
            try:
                ...
                loaded_count += 1
                logging.info(f"✅ تم تحميل بوت المستخدم: {user_id} من config")
                logging.info(f"   📡 المصدر: {user_config['source_channel']}")
                logging.info(f"   🎯 الهدف: {user_config['target_channel']}")
                
            except Exception as e:
                logging.error(f"❌ خطأ في تحميل بوت {user_id}: {str(e)}")
    
    # ثانياً: إذا كان config فارغاً، ابحث في مجلد users مباشرة
    if loaded_count == 0:
        logging.info("🔍 لم يتم العثور على مستخدمين في config، جاري البحث في مجلد users...")
                ...
                if user_file.endswith(".json"):
                    ...
                    try:
                        ...
                        if user_config and user_config.get("status") == "active":
                            ...
                            active_bots[user_id] = bot_instance
                            loaded_count += 1
                            logging.info(f"✅ تم تحميل بوت المستخدم: {user_id} من مجلد users")
                            logging.info(f"   📡 المصدر: {user_config['source_channel']}")
                            logging.info(f"   🎯 الهدف: {user_config['target_channel']}")
                    except Exception as e:
                        logging.error(f"❌ خطأ في تحميل بوت {user_id}: {str(e)}")
            
            # حفظ config بعد إضافة المستخدمين
            if loaded_count > 0:
                save_config(config)
                logging.info(f"📝 تم تحديث config.json بـ {loaded_count} مستخدم")
    ...
    # إرسال إشعار بعودة البوت للعمل إذا كان هناك بوتات نشطة
    if loaded_count > 0:
    ...
    try:
        ...
        try:
            bot.send_message(
                ...
                f"💾 تم حفظ إعداداتك - عند إعادة التشغيل سيعمل البوت تلقائياً"
            )
        except:
            pass
        ...
        bot.reply_to(message,
            ...
            f"💾 **جميع إعداداتك محفوظة - عند إعادة التشغيل سيعمل البوت تلقائياً**",
            parse_mode="Markdown"
        )
    ...
    if user_id in active_bots:
        ...
        try:
            ...
            bot.send_message(
                ...
                f"أو انتظر إعادة التشغيل التلقائي"
            )
        except:
            pass
        ...
        if user_id in config["active_users"]:
            ...
            save_config(config)
            logging.info(f"📝 تم إزالة المستخدم {user_id} من config.json")
...
💾 **الحفظ التلقائي:**
• جميع إعداداتك محفوظة في ملفات
• عند إعادة تشغيل البوت، يتم تحميلها تلقائياً
• لا حاجة لإعادة إدخال القنوات بعد إعادة التشغيل
...
def main():
    ...
    print("💾 حفظ تلقائي للإعدادات واستعادتها")
    print("="*70)
